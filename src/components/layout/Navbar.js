...
import getSiteMenuItems from '@/actions/menuItems';
  ...
  return (
    <div className="flex shrink-0 items-center">
      <Link href="/" className="h-9 lg:hidden">
        <Image src={logoUrl} alt="Banter Logo Icon" className="h-auto w-9" width={100} height={100} />
      </Link>
      <Link href="/" className="hidden h-9 lg:block">
        <Image src={logoUrl} alt="Banter logo" className="h-9 w-auto" width={100} height={100}/>
      </Link>
    </div>
  )
}
...
export default function Navbar() {
  const [siteMenuItems, setSiteMenuItems] = useState([]);
  ...
  const fetchData = async () => {
    const res = await getSiteMenuItems(window.location.host);
    const currentSiteAttributes = res?.attributes;
    ...
    setLogoUrl(logoUrl);
    const categories = currentSiteAttributes.categories ? currentSiteAttributes.categories?.data.map((category) => {
      return {
          ...
          <div className="relative z-10 border-b border-gray-300/60">
            <nav className="mx-auto flex h-20 max-w-7xl items-center bg-white px-4 sm:px-6 lg:border-0 lg:px-8">
              ...
              <div className="flex w-full items-center justify-between">
                ...
                <DesktopNavigation menu={siteMenuItems}/>
                <Search />
